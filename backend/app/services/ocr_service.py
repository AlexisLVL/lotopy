...
from app.services.model_predict import predict_digit  # 👈 ajoute en haut du fichier
...
def read_cells_ocr(cells_grid, cell_ocr="tmp"):
    """Lit les chiffres avec le modèle CNN entraîné sur ta police."""
    result = []
    ...
    for r, row in enumerate(cells_grid):
        ...
        for c, cell in enumerate(row):
            ...
            # On découpe la cellule (utile si deux chiffres côte à côte)
            ...
            for i, part in enumerate(halves):
                centered = center_digit_image(part)
                prepped = prepare_for_ocr(centered, debug=True, debug_name=f"r{r}_c{c}_part{i}")

                digit = predict_digit(prepped)
                if digit is not None:
                    digits.append(str(digit))

                # 🔍 Debug image prédiction
                if DEBUG:
                    os.makedirs(os.path.join(OCR_DEBUG_DIR, cell_ocr), exist_ok=True)
                    debug_img = cv2.putText(prepped.copy(), str(digit or "?"), (5, 30),
                                            cv2.FONT_HERSHEY_SIMPLEX, 1, (128, 128, 128), 2)
                    cv2.imwrite(os.path.join(OCR_DEBUG_DIR, f"{cell_ocr}/r{r}_c{c}_part{i}_{digit or 'none'}.png"), debug_img)

            # 🔢 Reconstruction du nombre final
            number = None
            if digits:
                try:
            ...
            # 🔹 Si le chiffre est simple, on complète la dizaine en fonction de la colonne
